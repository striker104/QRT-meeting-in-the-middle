...
# --- 2. File Paths ---
SCHEDULES_BASE_DIR = Path("/shared/challenge_data/schedules")
emissions_file = "/shared/challenge_data/emissions.csv"
...
def get_schedule_files_for_window(start_date: date, end_date: date) -> list[Path]:
    """
    Generates a list of file paths that exist within the given date window,
    following the /YYYY/MM/DD.csv structure.
    """
...
def find_best_meeting(input_json_path, weight_co2):
    ...
    for city_code in POTENTIAL_MEETING_CITIES:
        ...
        if possible and attendee_avg_scores:
            ...
            print(f"  SUCCESS: Score for {city_code}: {total_score:.2f} (Avg CO2: {total_co2:.2f}, Avg Fairness: {std_dev_travel:.2f}) (Took {elapsed_s:.2f}s)")
