...
def get_lazy_schedules():
    """
    Loads all schedules, cleans them, and filters for Passenger/Operating flights.
    """
    SCHEDULE_COLS = [
...
def find_flights(home_code, meeting_city_code, window_start, window_end):
    ...
    direct_flights_with_co2 = direct_flights_lazy.join(
        ...
        pl.col("CARRIER"), pl.col("FLTNO"), pl.col("DEPAPT"), pl.col("ARRAPT")
    )
    ...
    one_stop_flights = valid_connections.select(
        ...
        pl.col("CARRIER"), pl.col("FLTNO"), pl.col("DEPAPT"),
    ...
    *** THIS FUNCTION IS NOW CACHED ***
    ...
    if home_code == meeting_city_code:
        result_df = pl.DataFrame({
            ...
            "first_arrival": [None],
            "last_departure": [None],
            "Outbound_Route": ["Local"], "Inbound_Route": ["Local"],
            "outbound_carrier": [None], "outbound_fltno": [None], "outbound_dep_utc": [None], "outbound_dep_apt": [None], "outbound_arr_apt": [None],
            "inbound_carrier": [None], "inbound_fltno": [None], "inbound_dep_utc": [None], "inbound_dep_apt": [None], "inbound_arr_apt": [None],
        })
...
def get_final_json_for_city(city_code, rank, phase_1_score, scenario_dict, optimize_span=False):
    ...
    # *** NEW: Build the "itinerary" list in your requested format ***
    itinerary_list = []
    ...
    output_object = {
        ...
        "itinerary": itinerary_list  # <-- HERE IS YOUR NEW FORMAT
    }
    
    return output_object

# --- 8. Setup: This runs ONCE ---
print("Initializing Lazy DataFrames and Compatibility Map...")
...
print("Global data is ready. Script is ready to be called.")


# --- 9. Main Function ---
def find_best_meeting(input_json_path, weight_co2):
    """
    This is the main function for our API.
    It takes a json file path and a CO2 weight (0.0 to 1.0)
    ...
    for city_code in POTENTIAL_MEETING_CITIES:
        ...
        if possible and attendee_avg_scores:
            ...
            total_score = (weight_co2 * total_co2) + (weight_fairness * std_dev_travel)
...
# --- 9. Example of how to call our new function ---
...
if __name__ == "__main__":
    ...
    final_results = find_best_meeting(input_file, user_co2_weight)
